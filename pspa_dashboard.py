...
def save_credentials(username, password):
    with open(config_file, "w") as f:
        json.dump({"user": username, "password": password}, f)

def load_credentials():
    if os.path.exists(config_file):
        with open(config_file, "r") as f:
            return json.load(f)
    return {"user": "admin", "password": "secure123"}

credentials = load_credentials()

# --- Authentication ---
st.sidebar.markdown("### User Login")
USER = st.sidebar.text_input("Set Username", value=credentials["user"])
PASSWORD = st.sidebar.text_input("Set Password", value=credentials["password"])
if st.sidebar.button("Save Credentials"):
    save_credentials(USER, PASSWORD)
    st.sidebar.success("Credentials saved for future sessions.")

user = st.sidebar.text_input("Username to login")
password = st.sidebar.text_input("Password to login", type="password")
if user != USER or password != PASSWORD:
    st.error("Access denied: please enter valid credentials.")
    st.info("Default login: Username='admin', Password='secure123'. You can change them above.")
    st.stop()
...
st.markdown("**Version: 21/07/2025.**")
