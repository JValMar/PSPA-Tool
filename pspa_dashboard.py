...
class PSPAPDF(FPDF):
    ...
    def header(self):
        ...
        self.cell(0, 8, f"Project: {self.project_name}", ln=True, align="L")
        self.cell(0, 6, f"Date: {date.today()}", ln=True, align="L")
        self.ln(2)
    ...
    with col2:
        ...
        if uploaded_json is not None:
            try:
                ...
                st.success("Previous evaluation loaded. Reloading...")
                st.rerun()
...
if st.button("📄 Generate PDF"):
    ...
    pdf.cell(0, 10, "Domain Scores", ln=True)
    ...
    for d, s in domain_scores.items():
        ...
        pdf.set_text_color(0,0,0)
        pdf.cell(0, 8, f"{d} - {s:.1f}/10 ({ranking.upper()})", ln=True, fill=True)

    # Lowest questions
    pdf.ln(4)
    pdf.set_font("Arial", "B", 12)
    pdf.set_text_color(0,0,0)
    pdf.cell(0, 8, "Lowest Rated Questions", ln=True)
    pdf.set_font("Arial", "", 11)
    for d, q in lowest_questions.items():
        pdf_add_safe_multicell(pdf, f"{d}: {q}", w=0, h=6, txt_color=(200,0,0), italic=False)

    # Improvement plan
    pdf.ln(4)
    pdf.set_font("Arial", "B", 12)
    pdf.set_text_color(0,0,0)
    pdf.cell(0, 8, "Improvement Plan", ln=True)

    for d in domain_scores.keys():
        pdf.set_font("Arial", "B", 11)
        pdf.set_text_color(0,0,0)
        pdf.cell(0, 7, d, ln=True)
