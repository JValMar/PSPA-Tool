...
with pd.ExcelWriter(excel_buffer, engine='xlsxwriter') as writer:
    ...
    for idx, col in enumerate(df_summary.columns):
        ...
        summary_ws.set_column(idx, idx, col_width)
    ...
    # Insert Radar Chart
    summary_ws.insert_image('H2', 'radar_chart.png', {'image_data': img_buffer})

    # Write detailed questions
    df_questions = pd.DataFrame(questions_data)
    df_questions.to_excel(writer, index=False, sheet_name='Questions')
    questions_ws = writer.sheets['Questions']

    # Auto-adjust questions columns
    for idx, col in enumerate(df_questions.columns):
        col_width = max(df_questions[col].astype(str).map(len).max(), len(col)) + 2
        questions_ws.set_column(idx, idx, col_width)

    # Add header format for questions
    for col_num, value in enumerate(df_questions.columns.values):
        questions_ws.write(0, col_num, value, header_format)

st.download_button(
    "📊 Download Excel (.xlsx)",
    excel_buffer.getvalue(),
    file_name=f"{date.today()}_{project_name.replace(' ', '_')}_PSPA_Report.xlsx",
